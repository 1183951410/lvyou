...
public class LYMemory2Activity extends Activity {
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_MENU) {
			Intent intent = new Intent(LYMemory2Activity.this,
		...
		public void onClick(View v) {
			...
			if (v == bt2) {
				if (otherid.equals(hostid) || hostid == null) {
					Toast.makeText(LYMemory2Activity.this, "添加失败，未登录或者ID冲突请检查",
			...
			if (v == btnsendmsg) {
				if (hostid == null) {
					Toast.makeText(LYMemory2Activity.this, "未登录，请登录后再回复",
							Toast.LENGTH_LONG).show();
				} else {
					pd.show();
					Thread t = new Thread(r2);
					t.start();
					Toast.makeText(LYMemory2Activity.this, "回复成功",
							Toast.LENGTH_LONG).show();
				}
			}
		}
		...
		public void run() {
			...
			try {
				URL url = new URL("http://" + GLOBAL.IP
						+ ":8080/Lvyou/LYAddAttentionServlet");
	...
	Handler ha = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			if (result.equals("error")) {
				...
				Toast.makeText(LYMemory2Activity.this, "添加成功",
						Toast.LENGTH_LONG).show();
	...
	Runnable r2 = new Runnable() {
		...
		public void run() {
			...
			try {
				URL url = new URL("http://" + GLOBAL.IP
						+ ":8080/Lvyou/LYReplyMemoryServlet");
				HttpURLConnection htc = (HttpURLConnection) url
						.openConnection();
				htc.setDoInput(true);
				htc.setDoOutput(true);
	...
	private class myadapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			View v = LayoutInflater.from(c)
					.inflate(R.layout.lyreplyitem2, null);
	...
	Runnable r1 = new Runnable() {
		...
		public void run() {
			...
			try {
				URL url = new URL("http://" + GLOBAL.IP
						+ ":8080/Lvyou/LYAddFriendsServlet");
				HttpURLConnection htc = (HttpURLConnection) url
						.openConnection();
				htc.setDoInput(true);
				htc.setDoOutput(true);
	...
	Handler ha1 = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			if (result.equals("error")) {
				...
				Toast.makeText(LYMemory2Activity.this, "验证信息已经发送",
						Toast.LENGTH_LONG).show();
			}
		};
	};
