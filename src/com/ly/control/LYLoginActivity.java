...
public class LYLoginActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Cursor c = sd.rawQuery("select user_id _id,user_name,user_pswd from "
	...
	private OnClickListener l = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (cb.isChecked() && v == btn) {
				...
				sd.execSQL("insert into " + DBHelper.table_name
						+ " values(null,?,?)", new String[] { s, s1 });
				sd.close();
	...
	Runnable r = new Runnable() {
		...
		public void run() {
			...
			try {
				URL url = new URL(
						"http://"+GLOBAL.IP+":8080/Lvyou/LoginServlet");
	...
	Handler h = new Handler() {
		public void handleMessage(Message msg) {
			...
			if (re.equals("error")) {
				...
				Toast.makeText(LYLoginActivity.this, "欢迎" + ss[2] + "进入驴友天下行",
						Toast.LENGTH_LONG).show();
				Intent intent = new Intent(LYLoginActivity.this,
